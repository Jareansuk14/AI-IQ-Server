...
const handlePostbackEvent = async (event) => {
  try {
    ...
    switch (action) {
      ...
      // ส่วนปรับปรุงใน handlePostbackEvent - Forex Analysis
      case 'forex_analysis':
        ...
        try {
          ...
          // ✅ 1. ใช้ replyMessage แทน pushMessage (Priority สูงกว่า)
          // ✅ 2. รวมข้อความ loading + ผลลัพธ์เป็นการส่งครั้งเดียว
          ...
          // 🔄 ส่งข้อความ "กำลังประมวลผล..." ก่อน
          await lineService.replyMessage(event.replyToken, {
            type: 'text',
            text: `🔄 กำลังวิเคราะห์ ${forexPair}...\n\n📊 ระบบกำลังวิเคราะห์ข้อมูลทางเทคนิค\n⏳ กรุณารอสักครู่ (ประมาณ 10-15 วินาที)`
          });
          ...
          // 🚀 ส่งผลลัพธ์แบบรวมกัน (ไม่แยกส่ง)
          // รอสักครู่ให้ข้อความ "กำลังประมวลผล" แสดงผลก่อน
          await new Promise(resolve => setTimeout(resolve, 2000));

          // ✅ ใช้ pushMessage เพียงครั้งเดียวแต่ส่งหลายข้อความรวมกัน
          await lineService.pushMessage(userId, [
            // ส่งรูปภาพและข้อความพร้อมกัน
            {
              type: 'image',
