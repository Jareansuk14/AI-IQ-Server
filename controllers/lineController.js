//AI-Server/controllers/lineController.js - อัปเดตสำหรับระบบติดตามผล
...
const { 
  ...
  createContinueTradingMessage  // เพิ่มบรรทัดนี้
...
// ฟังก์ชันสำหรับตรวจสอบคำสั่งพิเศษ (อัปเดตเพื่อตรวจสอบสถานะการติดตาม)
const handleSpecialCommand = async (event) => {
  ...
  try {
    ...
    // หากกำลังติดตามผล ให้แจ้งเตือน (ยกเว้นคำสั่งดูเครดิต)
    if (isTracking && !['เครดิต', 'credit', 'เช็คเครดิต'].includes(text)) {
      return lineService.replyMessage(event.replyToken, {
        type: 'text',
        text: '⏳ คุณกำลังติดตามผลการเทรดอยู่\n\n🎯 กรุณารอจนกว่าจะเสร็จสิ้น\n📊 หรือพิมพ์ "ยกเลิกติดตาม" เพื่อหยุดการติดตาม'
      });
    }

    // คำสั่งยกเลิกการติดตาม
    if (text === 'ยกเลิกติดตาม' || text === 'ยกเลิก' || text === 'cancel') {
      if (isTracking) {
        try {
          await trackingService.cancelTracking(event.source.userId);
          return lineService.replyMessage(event.replyToken, {
            type: 'text',
            text: '✅ ยกเลิกการติดตามผลแล้ว\n\n📊 คุณสามารถเลือกคู่เงินใหม่ได้'
          });
        } catch (error) {
          return lineService.replyMessage(event.replyToken, {
            type: 'text',
        ...
        return lineService.replyMessage(event.replyToken, {
          type: 'text',
          text: '❌ ไม่พบการติดตามที่กำลังดำเนินการ'
        });
      }
    }
    ...
    // คำสั่ง AI-Auto (ตรวจสอบสถานะการติดตาม)
...
// อัปเดตฟังก์ชันจัดการ Postback Events
const handlePostbackEvent = async (event) => {
  try {
    ...
    switch (action) {
      ...
      // อัปเดต case สำหรับการวิเคราะห์ Forex พร้อมระบบติดตาม
      case 'forex_analysis':
        ...
        try {
          ...
          // ตรวจสอบว่ากำลังติดตามผลอยู่หรือไม่
          const isTracking = await trackingService.isUserTracking(event.source.userId);
          if (isTracking) {
            return lineService.replyMessage(event.replyToken, {
              type: 'text',
          ...
          try {
            ...
            await lineService.pushMessage(event.source.userId, {
              type: 'text',
              text: `⚠️ ไม่สามารถเริ่มระบบติดตามผลได้: ${trackingError.message}`
            });
          }
      ...
      // เพิ่ม case ใหม่สำหรับการตอบคำถาม "เทรดต่อหรือไม่?"
      case 'continue_trading':
        try {
          // แสดงการ์ดเลือกคู่เงิน
          const forexMessage = createForexPairsMessage();
          return lineService.replyMessage(event.replyToken, forexMessage);
        } catch (error) {
          console.error('Error continue trading:', error);
          return lineService.replyMessage(event.replyToken, {
            type: 'text',
      ...
      case 'stop_trading':
        try {
          return lineService.replyMessage(event.replyToken, {
            type: 'text',
            text: '👋 ขอบคุณที่ใช้บริการ AI-Auto!\n\n🎯 หวังว่าจะได้พบกันใหม่เร็วๆ นี้\n💪 ขอให้โชคดีในการเทรดครั้งต่อไป!\n\n📱 พิมพ์ "AI-Auto" เมื่อไหร่ก็ได้เพื่อเริ่มใหม่'
          });
        } catch (error) {
          console.error('Error stop trading:', error);
          return lineService.replyMessage(event.replyToken, {
            type: 'text',
            text: '❌ เกิดข้อผิดพลาด'
          });
...
// ฟังก์ชันหลักสำหรับการจัดการข้อความ (อัปเดตเพื่อตรวจสอบการติดตาม)
